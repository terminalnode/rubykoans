...
class AboutRegularExpressions < Neo::Koan
  ...
  def test_asterisk_means_zero_or_more
    ...
    #
    # It will never fail on it's own, but as part of a more complex
    # regular expression it can.
    assert_equal nil, "abbcccddddeeeee"[/xz*/]

    # THINK ABOUT IT:
    # We say that the repetition operators above are "greedy."
    # Why?
    #
    # + and * will match as much as possible, it will not stop at the
    # shortest possible match. for example /<.*>/ will not match the first
    # tag it sees, it will match the first < and anything up until and
    # including the last > in the string.
    assert_equal "<super> <greedy>", "<super> <greedy>"[/<.*>/]
  end
  ...
  def test_parentheses_also_capture_matched_content_by_number
    # note to self: indexing starts at 1, not 0
  ...
  #
  # A character class will only match single characters, an alternation
  # can match whole expressions (such as /James/).
  ...
  def test_sub_is_like_find_and_replace
    # matches first t\w* and replaces it with the first character in the match
    assert_equal "one t-three", "one two-three".sub(/(t\w*)/) { $1[0, 1] }
  end

  def test_gsub_is_like_find_and_replace_all
    # same as above but matches all t\w*
    assert_equal "one t-t", "one two-three".gsub(/(t\w*)/) { $1[0, 1] }
    # self test
    assert_equal "one ha-ha", "one two-three".gsub(/(t\w*)/) { "ha" }
  end
end
